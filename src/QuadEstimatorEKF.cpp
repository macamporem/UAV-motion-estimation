...
//  ekfState(6) = ekfState(6) + dtIMU * gyro.z;    // yaw
    ...
    //Inverse transformation from body rates to Euler-angle rates using current Euler angles
    Mat3x3F InvR = Mat3x3F::Zeros();
    InvR(0,0) = 1;
    InvR(0,1) = sin(rollEst) * tan(pitchEst);
    InvR(0,2) = cos(rollEst) * tan(pitchEst);
    InvR(1,0) = 0;
    InvR(1,1) = cos(rollEst);
    InvR(1,2) = -sin(rollEst);
    InvR(2,0) = 0;
    InvR(2,1) = sin(rollEst) / cos(pitchEst);
    InvR(2,2) = cos(rollEst) / cos(pitchEst);

    //Euler_Angle_Rates
    V3F Euler_Angle_Rates = InvR * gyro;

    //better integration (prediction) method that uses the current
    //attitude estimate (rollEst, pitchEst and ekfState(6))
    float predictedPitch = pitchEst    + dtIMU * Euler_Angle_Rates.y;
    float predictedRoll =  rollEst     + dtIMU * Euler_Angle_Rates.x;
    ekfState(6) =          ekfState(6) + dtIMU * Euler_Angle_Rates.z;
    
  // doesn't change --> normalize yaw to -pi .. pi
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    VectorXf xt = curState;
    VectorXf g_xt_ut_dt = xt;

    g_xt_ut_dt(0) = xt(0) + dt * xt(3);
    g_xt_ut_dt(1) = xt(1) + dt * xt(4);
    g_xt_ut_dt(2) = xt(2) + dt * xt(5);
    
    //The acceleration vector passed to the function is given in the body frame.
    //To transform to inertial frame we use need to rotate using current attitude.
    V3F accel_inertial = attitude.Rotate_BtoI(accel);
    
    g_xt_ut_dt(3) = xt(3) + dt * accel_inertial.x;
    g_xt_ut_dt(4) = xt(4) + dt * accel_inertial.y;
    //note that accel (acceleration of the vehicle, in body frame) DOESNT including gravity
    g_xt_ut_dt(5) = xt(5) + dt * accel_inertial.z - dt * CONST_GRAVITY;

    predictedState = g_xt_ut_dt;
  /////////////////////////////// END STUDENT CODE ////////////////////////////
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    // first calculate the transition Jacobian:
    // gPrime is already set to the identity
    // the only terms that need to be added are the 'dt' in the first three rows
    // and the 'RBGprime x accel x dt' terms in the next three rows

    // 'dt' terms in rows 0,1,2
    gPrime(0,3) = dt;
    gPrime(1,4) = dt;
    gPrime(2,5) = dt;
    
    // RBGprime x accel x dt terms in rows 3,4,5
